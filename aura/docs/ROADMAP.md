...
- ~~**Casamentos — site do casal**~~ — **entregue e em produção** (`/casamento` e `/casamento/<code>`): código de 6 dígitos como credencial, simulador da janela do evento para o convidado, pré-reserva com preço travado virando `rate_quote` no funil, e painel dos noivos (ocupação por categoria + capa/mensagem/cores). Falta: lista de presentes/convidados.
- **Widget embutível do site do casal** *(ideia levantada em 26/08, não iniciada)* — permitir que o casal cole o simulador de reserva no site de casamento dele (Wix, Zankyou, domínio próprio). Tecnicamente barato; **quatro travas reais antes de abrir**, medidas no código:
  1. **`X-Frame-Options: SAMEORIGIN` em `/(.*)`** (`next.config.mjs:17`) — nenhum iframe de terceiro abre hoje. Como cada casal tem um domínio, a política não cabe em header estático: `frame-ancestors` por casamento, resolvido no server component da página (que já tem o `siteConfig` na mão), **não** no middleware — ler `weddings` no Edge encareceria justamente o caminho quente.
  2. **O `guestCode` vira público.** Colado num site aberto, ele deixa de ser "o código do convite". E `/prebook` **não paga rate-limit em caminho de sucesso**: a anti-duplicata é só por telefone digitado. Script com telefones falsos zera o soft-block e todas as categorias viram "esgotado" para os convidados de verdade. **Pré-reservar precisa de mais do que o código** antes de o embed existir.
  3. **O contador de rate-limit é compartilhado com o login do staff** (`src/lib/login-attempts.ts:5-8`, por decisão de projeto). O teto do login é 15 (`api/admin/auth/login/route.ts:17`) e o da página do casamento é 30 (`casamento/[code]/page.tsx:28`): **15 aberturas com código errado do mesmo IP travam o login da recepção por 15 minutos** — e um iframe deixado no ar depois do casamento gera miss a cada visualização, porque o cron promove para `completed` e `resolveCode` passa a devolver null. Namespace próprio de rate-limit + página de encerramento (200, sem `logAttempt`) são pré-requisito.
  4. **`coverPhotoUrl` aceita qualquer host https** (`wedding-site-service.ts:592`, regex sem allowlist). Numa página pública embutida, isso exporta IP e User-Agent de todo convidado para um terceiro arbitrário. Amarrar ao bucket da propriedade.

  Bônus de egress, que vale mesmo sem o embed: `resolveCode` roda 2× por render do convidado e 3× no painel do casal (~13 queries por abertura), a capa sobe **sem compressão** (até 5 MB) e entra como CSS `url()` fora do otimizador da Vercel, e o link copiado no admin usa `?code=` — que executa a server action e só então redireciona para `/casamento/<code>`, refazendo tudo.
