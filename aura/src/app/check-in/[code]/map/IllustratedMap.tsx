...
export function IllustratedMap({
    ...
    // Declutter de rótulos: mostra o nome só quando não há outro nome colado.
    // Em aglomerados, os pins viram ícone-só (nome continua no toque e na lista).
    const showLabelFor = useMemo(() => {
        const shown: { x: number; y: number }[] = [];
        const map: Record<string, boolean> = {};
        for (const a of placed) {
            const x = a.mapPin!.pixelX!, y = a.mapPin!.pixelY!;
            const collides = shown.some(s => Math.abs(s.x - x) < 0.14 && Math.abs(s.y - y) < 0.055);
            map[a.id] = !collides;
            if (!collides) shown.push({ x, y });
        }
        return map;
    }, [placed]);
    ...
    return (
        <div className="relative w-full rounded-3xl overflow-hidden border border-border bg-secondary shadow-sm">
            ...
            {/* Pins das áreas — dot compacto sempre; nome só quando há espaço */}
            {placed.map(area => {
                const color = area.pinColor || DEFAULT_PIN_COLOR;
                const isSelected = area.id === selectedId;
                const showLabel = showLabelFor[area.id] || isSelected;
                return (
                    <button
                        key={area.id}
                        onClick={() => onAreaClick(area)}
                        className="absolute -translate-x-1/2 -translate-y-1/2 group focus:outline-none"
                        style={{ left: `${(area.mapPin!.pixelX ?? 0) * 100}%`, top: `${(area.mapPin!.pixelY ?? 0) * 100}%`, zIndex: isSelected ? 30 : showLabel ? 20 : 15 }}
                    >
                        {/* Dot com ícone */}
                        <div
                            className="w-7 h-7 rounded-full ring-2 ring-white shadow-lg grid place-items-center text-[13px] transition-transform group-hover:scale-110 group-active:scale-95"
                            style={{ background: color }}
                        >
                            {area.pinIcon || "📍"}
                        </div>
                        {/* Rótulo de nome (declutter) */}
                        {showLabel && (
                            <span
                                className="absolute left-1/2 -translate-x-1/2 top-[30px] whitespace-nowrap text-[10px] font-bold text-white px-1.5 py-0.5 rounded-md shadow max-w-[100px] truncate"
                                style={{ background: color }}
                            >
                                {area.name}
                            </span>
