...
export function FlutuacoesTab({ propertyId, bundle, onRefresh }: {
  ...
  const save = async (mode: "strict" | "overwrite" | "fill") => {
    ...
    try {
      ...
      toast.success(
        mode === "overwrite"
          ? "Flutuação aplicada — as antigas foram aparadas nessas datas."
          : data?.created > 1 ? `Flutuação aplicada em ${data.created} trechos livres.` : "Flutuação aplicada ao período."
      );
      setForm(EMPTY);
      setConflicts(null);
      await onRefresh();
