...
export interface StayCardProps {
  stay: StayRow;
  mode: "ativas" | "futuras";
  /** "compact" esconde status, pré-check-in e o botão Ver ficha — o cartão inteiro abre a ficha. */
...
export function StayCard({ stay: s, mode, variant = "full", onOpen, onWhatsapp, onCheckIn, onCheckOut, onCancel, onCopyLink, opening, checkingIn }: StayCardProps) {
  ...
  // No compacto o cartão inteiro é o botão "Ver ficha" — os controles internos
  // param a propagação para não abrir a ficha por engano.
  const stop = (fn: () => void) => (e: React.MouseEvent) => { e.stopPropagation(); fn(); };
  const cardProps = compact
    ? {
        role: "button" as const,
        tabIndex: 0,
        "aria-label": `Abrir ficha de ${shortName(s.guestName)}`,
        onClick: () => onOpen(s),
  ...
  return (
    <article
      {...cardProps}
      className={`ak-card${compact ? " ak-press ak-focus" : ""}`}
      data-pad={compact ? "14" : "16"}
      style={{ display: "flex", flexDirection: "column", gap: compact ? 10 : 12, borderColor: status.tone === "red" ? T.redBorder : undefined, cursor: compact ? "pointer" : undefined }}
    >
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
        ...
        <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
          ...
          {compact && mode === "futuras" && s.status === "pending" && onCopyLink && (
            <IconButton icon={Copy} label="Copiar link do check-in" size="sm" onClick={stop(() => onCopyLink(s.accessCode))} />
          )}
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 8, paddingTop: 2 }}>
        ...
        {mode === "futuras" && onCheckIn && (
          <Button variant="primary" icon={LogIn} onClick={stop(() => onCheckIn(s))} loading={checkingIn} style={{ flex: 1 }}>Check-in</Button>
        )}
        {mode === "futuras" && onCancel && (
          <Button variant="danger" icon={Ban} onClick={stop(() => onCancel(s))} aria-label="Cancelar reserva" title="Cancelar reserva" />
        )}
      </div>
    </article>
  );
