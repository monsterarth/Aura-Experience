...
import { Clock, Hammer, AlertCircle, CheckCircle2, PlayCircle, Plus, Edit3, Settings2, Archive, Calendar as CalendarIcon, X, Moon, ChevronLeft } from "lucide-react";
                ...
                renderTasks={() => (
                    <div className="flex flex-col h-[100dvh] bg-background">
                        ...
                        <div className="p-6 border-b border-border bg-card shrink-0">
                            ...
                            <p className="text-sm text-muted-foreground mt-1">Visualize e gerencie as manutenções atribuídas a você.</p>
                        </div>
                        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-secondary/10">
                            {myInProgress.length > 0 && (
                                <div className="mb-8 block">
                                    <h3 className="font-bold text-sm uppercase tracking-widest text-primary flex items-center gap-2 mb-4"><PlayCircle size={16} /> Em Andamento</h3>
                                    {myInProgress.map(t => <TaskCard key={t.id} task={t} />)}
                                </div>
                            )}
                            {myPending.length > 0 && (
                                <div className="mb-8 block">
                                    <h3 className="font-bold text-sm uppercase tracking-widest text-foreground flex items-center gap-2 mb-4"><Hammer size={16} /> Pendentes</h3>
                                    {myPending.map(t => <TaskCard key={t.id} task={t} />)}
                                </div>
                            )}
                            {myWaiting.length > 0 && (
                                <div className="mb-4 block">
                                    <h3 className="font-bold text-sm uppercase tracking-widest text-orange-500 flex items-center gap-2 mb-4"><AlertCircle size={16} /> Requer Atenção</h3>
        ...
        <div className="flex-1 min-w-[300px] flex flex-col bg-muted/20 border border-border rounded-2xl overflow-hidden">
            ...
            <div className="p-4 flex-1 overflow-y-auto space-y-4 custom-scrollbar">
                {items.length === 0 ? (
                    ...
                    items.map(task => {
                        const isManager = userData?.role === 'admin' || userData?.role === 'super_admin' || userData?.role === 'maintenance' || userData?.role === 'manager';
                        return (
                            <div key={task.id} className={cn(
                                ...
                                {isManager && (
                                    <button onClick={() => { setSelectedTask(task); setIsManagerOpen(true); }} className="absolute top-4 right-4 p-2 bg-secondary text-muted-foreground hover:text-primary rounded-lg transition-colors opacity-0 group-hover:opacity-100">
                                        <Edit3 size={16} />
                                    </button>
                                )}
                                <div className="pr-10">
                                    ...
                                    {task.imageUrl && (
                                        <button className="relative mt-2 h-20 w-full rounded-lg overflow-hidden border border-border cursor-pointer group block text-left" onClick={(e) => { e.stopPropagation(); setEnlargedImage(task.imageUrl!); }}>
                                            <Image src={task.imageUrl} alt="Evidência" fill className="object-cover group-hover:scale-105 transition-transform" />
                                ...
                                <div className="space-y-2">
                                    {task.status === 'pending' ? (
                                        <div className="pt-2 border-t border-border">
                                            <button onClick={() => handleStartTask(task.id)} className="w-full flex items-center justify-center gap-2 bg-primary/10 hover:bg-primary text-primary hover:text-primary-foreground font-bold p-3 rounded-xl transition-all text-xs uppercase cursor-pointer">
                                                <PlayCircle size={16} /> Iniciar Manutenção
                                            </button>
                                        </div>
                                    ) : task.status === 'in_progress' ? (
                                        <div className="pt-2 border-t border-border">
                                            <button onClick={() => { setSelectedTask(task); setIsCompletionOpen(true); }} className="w-full flex items-center justify-center gap-2 bg-blue-500 hover:bg-blue-600 text-white font-bold p-3 rounded-xl transition-all text-xs uppercase">
                                                <CheckCircle2 size={16} /> Finalizar Tarefa
                                            </button>
                                        </div>
                                    ) : task.status === 'waiting_conference' ? (
                                        <div className="bg-orange-500/10 border border-orange-500/30 p-3 rounded-xl">
                                            ...
                                            <button onClick={() => { setSelectedTask(task); setIsCompletionOpen(true); }} className="w-full mt-2 bg-orange-500 hover:bg-orange-600 text-white font-bold p-2 text-[10px] uppercase tracking-widest rounded-lg">Tratar Problema</button>
                                        </div>
                                    ) : (
        ...
        <div className="h-full flex flex-col space-y-4 p-4 md:p-0">
            <div className="flex items-center justify-between mt-2 md:mt-0">
                ...
                <div className="flex gap-2">
                    <button onClick={() => setIsArchiveOpen(true)} className="px-4 py-2 bg-secondary text-foreground font-bold text-xs uppercase tracking-widest rounded-xl hover:bg-accent transition-colors flex items-center gap-2 shadow-sm border border-border">
                        <Archive size={16} /> Arquivo
                    </button>
                    <button onClick={() => { setSelectedTask(null); setIsManagerOpen(true); }} className="px-4 py-2 bg-primary text-primary-foreground font-bold text-xs uppercase tracking-widest rounded-xl hover:opacity-90 transition-opacity flex items-center gap-2 shadow-sm">
            ...
            <div className="flex-1 overflow-x-auto pb-4 custom-scrollbar">
                <div className="flex gap-4 h-full min-w-max">
                    ...
                    <KanbanColumn title="Requer Atenção" icon={AlertCircle} colorClass="text-orange-500" items={waitingTasks} />
                </div>
            </div>

            <MaintenanceTaskManagerModal isOpen={isManagerOpen} onClose={() => { setIsManagerOpen(false); setSelectedTask(null); }} propertyId={property.id} task={selectedTask} cabins={cabins} structures={structures} technicians={technicians} />
            <MaintenanceCompletionModal isOpen={isCompletionOpen} onClose={() => { setIsCompletionOpen(false); setSelectedTask(null); }} propertyId={property.id} task={selectedTask} cabins={cabins} structures={structures} />
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
                    <div className="bg-card border border-border w-full max-w-3xl rounded-[32px] overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
                        <header className="p-6 border-b border-border flex justify-between items-center shrink-0">
                            <div>
                                <h2 className="text-xl font-black uppercase text-foreground tracking-tighter flex items-center gap-2"><Archive size={20} className="text-muted-foreground" /> Arquivo de Manutenção</h2>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar bg-secondary/20">
                            {recentCompletedTasks.length === 0 ? (
                                <div className="text-center py-12 text-muted-foreground"><Archive size={40} className="mx-auto mb-4 opacity-20" /><p className="text-sm font-bold uppercase tracking-widest">Nenhuma OS finalizada esta semana.</p></div>
                            ) : (
            ...
            {enlargedImage && (
                <div className="fixed inset-0 z-[200] bg-black/90 flex flex-col items-center justify-center p-4" onClick={() => setEnlargedImage(null)}>
                    <button onClick={() => setEnlargedImage(null)} className="absolute top-4 right-4 text-white bg-black/50 p-2 rounded-full backdrop-blur-md z-10 hover:bg-black/80 transition-colors pointer-events-auto"><X size={24} /></button>
