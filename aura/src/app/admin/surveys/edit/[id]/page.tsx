...
export default function EditSurveyTemplatePage() {
  ...
  const handleSave = async () => {
    ...
    else { alert(result.error); setSaving(false); }
  };
    ...
    <div className="flex flex-col h-full bg-muted/20 pb-20 relative">
      <header className="flex items-center justify-between px-6 py-4 bg-background border-b sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight text-foreground">Editar Pesquisa</h1>
            <p className="text-sm text-muted-foreground">Modificando: {title}</p>
          </div>
        </div>
        <Button onClick={handleSave} disabled={saving} className="gap-2">
          {saving ? "Salvando..." : <><Save className="w-4 h-4" /> Atualizar Pesquisa</>}
        </Button>
      </header>
