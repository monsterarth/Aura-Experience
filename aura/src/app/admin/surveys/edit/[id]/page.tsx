...
import {
  Plus, Trash2, Save, ArrowLeft, ArrowUp, ArrowDown, Gift, Settings, ListOrdered, Loader2, X, CircleDot, CheckSquare, Globe
} from "lucide-react";
...
export default function EditSurveyTemplatePage() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const result = await SurveyService.updateTemplate(property.id, templateId, {
            ...
            {reward.hasReward && (
              <div className="space-y-4 animate-in fade-in">
                <select className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={reward.type} onChange={(e) => setReward({ ...reward, type: e.target.value as any })}>
                  ...
                  <option value="freebie">Brinde</option>
                </select>
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-muted-foreground">Mensagem (PT)</label>
                  <textarea className="flex min-h-[80px] w-full rounded-md border border-input bg-background px-3 py-2 text-sm resize-none" value={reward.description} onChange={(e) => setReward({ ...reward, description: e.target.value })} />
                </div>
                <div className="space-y-1.5">
