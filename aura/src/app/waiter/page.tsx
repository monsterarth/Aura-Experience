// src/app/waiter/page.tsx — standalone mobile app (no admin layout)
"use client";
...
function WaiterOrderDialog({
  propertyId, tables, attendances, onClose, onSubmit,
}: {
  propertyId: string;
  ...
  const hasPendingFlavors = menuItems.some(i =>
    selections[i.id]?.selected && (i.flavors?.length ?? 0) > 0 && !selections[i.id]?.flavor
  );
  ...
  const handleSubmit = async () => {
    ...
    const items = menuItems
      ...
      .map(i => ({ menuItemId: i.id, name: i.name, quantity: 1, unitPrice: i.price || 0, flavor: selections[i.id]?.flavor, guestName: selectedAttendanceName ?? undefined }));
          ...
          {selectedTableId && (
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {tableAttendances.length === 0 && <p className="text-xs text-foreground/30">Nenhum hóspede sentado nesta mesa</p>}
              </div>
            </div>
          )}
          {categories.map(cat => {
...
function AssignTableDialog({
  attendance, tables, session, propertyId, actorName, onClose, onAssigned,
}: {
  ...
  const handleAssign = async (tableId: string) => {
    setLoading(true);
    try { await BreakfastSalonService.assignTable(attendance.id, tableId); onAssigned(); onClose(); }
...
function VisitorDialog({
  tables, session, propertyId, actorName, onClose, onAdded,
}: {
  tables: BreakfastTable[];
  session: BreakfastSession;
  propertyId: string;
  ...
  const handleAdd = async () => {
    ...
    setLoading(true);
    try { await BreakfastSalonService.addVisitor(propertyId, session.id, targetTableId, name.trim()); onAdded(); onClose(); }
    finally { setLoading(false); }
  };
...
export default function WaiterPage() {
  ...
  const handlePlaceOrder = async (tableId: string, attendanceId: string | null, items: any[]) => {
    ...
    await BreakfastSalonService.placeWaiterOrder(
      propertyId, att?.stayId ?? null, tableId, attendanceId, items,
      userData?.id ?? "waiter", actorName, att?.guestName ?? null, att?.cabinName ?? null
    );
    ...
    <RoleGuard allowedRoles={["super_admin", "admin", "reception", "kitchen", "waiter"]}>
      <div className="flex flex-col h-screen bg-background max-w-md mx-auto">
        ...
        <div className="px-4 pt-5 pb-3 border-b border-white/5">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setOrderDialogOpen(true)} disabled={!session || session.status !== 'open'}
                className="p-2.5 bg-primary/10 text-primary rounded-xl disabled:opacity-30" title="Novo pedido">
                <ChefHat size={16} />
              </button>
              {session?.status === 'open' ? (
                    ...
                    <div key={o.id} className="bg-secondary border border-white/5 rounded-2xl p-4">
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          ...
                          {o.guestName && <p className="text-[10px] text-foreground/40">{o.guestName} · {o.cabinName}</p>}
        ...
        {assignTarget && session && (
          <AssignTableDialog attendance={assignTarget} tables={tables} session={session} propertyId={propertyId} actorName={actorName}
            onClose={() => setAssignTarget(null)} onAssigned={() => { loadData(); setAssignTarget(null); }} />
        )}
        {moveTarget && session && (
          <AssignTableDialog attendance={moveTarget} tables={tables} session={session} propertyId={propertyId} actorName={actorName}
            onClose={() => setMoveTarget(null)} onAssigned={() => { loadData(); setMoveTarget(null); }} />
        )}
        {visitorDialogOpen && session && (
          <VisitorDialog tables={tables} session={session} propertyId={propertyId} actorName={actorName}
            onClose={() => setVisitorDialogOpen(false)} onAdded={() => { loadData(); setVisitorDialogOpen(false); }} />
        )}
        {orderDialogOpen && session && (
          <WaiterOrderDialog propertyId={propertyId} tables={tables} attendances={attendances}
            onClose={() => setOrderDialogOpen(false)} onSubmit={handlePlaceOrder} />
        )}
      </div>
    </RoleGuard>
  );
