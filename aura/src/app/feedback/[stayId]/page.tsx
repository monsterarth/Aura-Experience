...
export default function GuestFeedbackPage() {
  ...
  const handleSubmit = async () => {
    ...
    const result = await SurveyService.submitSurvey(
      stayContext.propertyId, stayContext.stay.id, stayContext.stay.guestId, template, answers
    );
  ...
  const renderNPS = (q: SurveyQuestion) => (
    <div key={q.id} className="mb-8 animate-in fade-in slide-in-from-bottom-2">
      ...
      <div className="flex justify-between text-xs text-muted-foreground font-medium px-1">
        <span>0 - Não recomendaria</span><span>10 - Com certeza</span>
      </div>
    </div>
  );
  ...
  const renderTextarea = (q: SurveyQuestion) => (
    <div key={q.id} className="mb-8 animate-in fade-in slide-in-from-bottom-2">
      ...
      {q.description && <p className="text-xs text-muted-foreground mb-3">{q.description}</p>}
      <Textarea 
        placeholder="Digite aqui seu feedback..." className="resize-none min-h-[120px] bg-background border-input shadow-sm"
        value={answers[q.id] || ""} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => handleAnswer(q.id, e.target.value)}
      />
    </div>
  );

  // NOVO: Renderizador de Múltipla Escolha e Escolha Única
  const renderChoice = (q: SurveyQuestion, isMultiple: boolean) => {
    ...
    return (
      <div key={q.id} className="mb-8 bg-card border rounded-xl p-5 shadow-sm animate-in fade-in slide-in-from-bottom-2">
        <h3 className="text-base font-medium text-foreground mb-1">{q.text}</h3>
        {q.description && <p className="text-xs text-muted-foreground mb-4">{q.description}</p>}
  ...
  if (success) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-muted-foreground mb-8 max-w-sm">Sua avaliação foi registrada. Ela é fundamental para continuarmos melhorando.</p>
  ...
  if (error || !template) return <div className="min-h-screen flex items-center justify-center p-6 text-center"><h1 className="text-xl font-bold text-destructive mb-2">Ops!</h1><p className="text-muted-foreground">{error || "Não conseguimos carregar a pesquisa."}</p></div>;

  return (
    <div className="min-h-screen bg-background flex flex-col pb-8">
      <header className="bg-card py-4 px-6 text-center border-b sticky top-0 z-10 shadow-sm">
        <h1 className="text-lg font-semibold tracking-tight text-foreground">{template.title || "Pesquisa de Satisfação"}</h1>
      </header>
      ...
      <main className="flex-1 max-w-lg w-full mx-auto p-6 flex flex-col mt-4">
        ...
        <div className="mt-6">
          <Button size="lg" className="w-full h-14 text-base font-semibold shadow-md rounded-xl" onClick={handleSubmit} disabled={submitting || !isFormValid()}>
            {submitting ? <><Loader2 className="w-5 h-5 mr-2 animate-spin" /> Enviando...</> : <>Enviar Avaliação <Send className="w-5 h-5 ml-2" /></>}
          </Button>
          {!isFormValid() && <p className="text-xs text-center text-muted-foreground mt-3">Por favor, responda pelo menos uma pergunta para enviar.</p>}
        </div>
      </main>
    </div>
  );
