...
export async function GET(request: Request) {
  const auth = await requireAuth(['super_admin', 'admin', 'manager', 'reception', 'governance', 'kitchen', 'maintenance', 'marketing', 'maid', 'houseman', 'waiter', 'porter', 'technician']);
  ...
  try {
    ...
    if (propertyId) {
      ...
      const { data: staffList, error: staffError } = await supabaseAdmin
        .from('staff')
        .select('id, fullName, role, active, profilePictureUrl, scheduleType, scheduleConfig, birthDate, email, phone, hireDate')
