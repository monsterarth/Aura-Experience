...
export const Sidebar = () => {
  ...
  return (
    <aside className="w-64 min-h-screen bg-[#141414] border-r border-white/5 flex flex-col p-4 animate-in slide-in-from-left duration-300">
      
      {/* HEADER SIDEBAR */}
      <div className="mb-8 px-2 flex items-center gap-2">
         <div className="w-8 h-8 bg-primary rounded-lg flex items-center justify-center font-black text-black">A</div>
         <span className="font-bold text-white tracking-tight">Aura Engine</span>
      </div>

      {/* SELETOR GLOBAL - Apenas para Super Admin */}
      {isSuperAdmin && (
        <div className="mb-6 p-3 bg-white/5 rounded-2xl border border-white/5 space-y-2">
          <label className="text-[9px] font-black text-white/40 uppercase tracking-widest px-1">Foco da Instância</label>
          <div className="relative group">
            ...
            <select 
              ...
              className="w-full bg-black/40 border border-white/10 p-2.5 pl-9 rounded-xl text-[10px] font-bold text-white outline-none appearance-none hover:border-primary/50 transition-all cursor-pointer uppercase"
            >
      ...
      <div className="pt-4 border-t border-white/5 mt-4">
        {property && !isSuperAdmin && (
            <div className="mb-4 px-2">
                <p className="text-[9px] font-bold text-white/20 uppercase">Propriedade</p>
